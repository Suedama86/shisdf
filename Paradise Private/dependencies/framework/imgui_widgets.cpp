...
#define ImGui_ANIMATIONS_SPEED 0.05f
#endif
...
bool ImGui::Checkbox(const char* label, bool* v, float spacing_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    float height = g.FontSize + style.ItemInnerSpacing.y * 2;
    float width = height;  // Checkbox will be square
    ...
    ImU32 colCheckMark = *v ? GetColorU32(ImVec4(0.510f, 0.282f, 1.000f, 1.0f)) : 0; // Purple color if checked, otherwise invisible

    // Render Checkbox
    window->DrawList->AddRectFilled(pos, pos + ImVec2(width, height), GetColorU32(colFrame), style.FrameRounding);

    // Render Checkmark if checked
    if (*v) {
        RenderCheckMark(window->DrawList, pos + ImVec2(4, 4), colCheckMark, height - 8); // Adjust padding as needed
    }

    // Render Label
    if (label_size.x > 0)
    {
        RenderText(pos + ImVec2(width + style.ItemInnerSpacing.x, style.ItemInnerSpacing.y), label);
    }
...
bool ImGui::RadioFrameIcon(ImFont* icon_font, const char* icon, int* v, int current_id, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(icon);  // Use icon for ID
    ...
    // Calculate item size (keeping padding for spacing)
    ...
    // Render the icon only, using a default color
    window->DrawList->AddText(icon_font, icon_font->FontSize, pos + ImVec2(size.x / 2 - icon_size.x / 2, style.FramePadding.y), GetColorU32(ImGuiCol_Text), icon);
...
bool ImGui::Combo(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ...
    ImVec4 custom_color = ImColor(195, 52, 235);

    // Save the original style colors
    ImGuiStyle& style = ImGui::GetStyle();
    ImVec4 original_header_color = style.Colors[ImGuiCol_Header];
    ImVec4 original_header_hovered_color = style.Colors[ImGuiCol_HeaderHovered];
    ImVec4 original_header_active_color = style.Colors[ImGuiCol_HeaderActive];

    // Set the custom style colors
    style.Colors[ImGuiCol_Header] = custom_color;
    style.Colors[ImGuiCol_HeaderHovered] = custom_color * ImVec4(1, 1, 1, 0.8f); // Slightly faded for hover
    style.Colors[ImGuiCol_HeaderActive] = custom_color * ImVec4(1, 1, 1, 0.6f);  // More faded for active
    ...
    {
        // Restore the original style colors before returning
        style.Colors[ImGuiCol_Header] = original_header_color;
        style.Colors[ImGuiCol_HeaderHovered] = original_header_hovered_color;
        style.Colors[ImGuiCol_HeaderActive] = original_header_active_color;
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, const void* arg1, const void* arg2)
{
    ...
    switch (data_type)
    {
        case ImGuiDataType_S8:
            if (op == '+') { *(ImS8*)output  = ImGuiClampOverflow(*(const ImS8*)arg1,  *(const ImS8*)arg2,  IM_S8_MIN,  IM_S8_MAX); }
            if (op == '-') { *(ImS8*)output  = ImSubClampOverflow(*(const ImS8*)arg1,  *(const ImS8*)arg2,  IM_S8_MIN,  IM_S8_MAX); }
            return;
        case ImGuiDataType_U8:
            if (op == '+') { *(ImU8*)output  = ImGuiClampOverflow(*(const ImU8*)arg1,  *(const ImU8*)arg2,  IM_U8_MIN,  IM_U8_MAX); }
            if (op == '-') { *(ImU8*)output  = ImSubClampOverflow(*(const ImU8*)arg1,  *(const ImU8*)arg2,  IM_U8_MIN,  IM_U8_MAX); }
            return;
        case ImGuiDataType_S16:
            if (op == '+') { *(ImS16*)output = ImGuiClampOverflow(*(const ImS16*)arg1, *(const ImS16*)arg2, IM_S16_MIN, IM_S16_MAX); }
            if (op == '-') { *(ImS16*)output = ImSubClampOverflow(*(const ImS16*)arg1, *(const ImS16*)arg2, IM_S16_MIN, IM_S16_MAX); }
            return;
        case ImGuiDataType_U16:
            if (op == '+') { *(ImU16*)output = ImGuiClampOverflow(*(const ImU16*)arg1, *(const ImU16*)arg2, IM_U16_MIN, IM_U16_MAX); }
            if (op == '-') { *(ImU16*)output = ImSubClampOverflow(*(const ImU16*)arg1, *(const ImU16*)arg2, IM_U16_MIN, IM_U16_MAX); }
            return;
        case ImGuiDataType_S32:
            if (op == '+') { *(ImS32*)output = ImGuiClampOverflow(*(const ImS32*)arg1, *(const ImS32*)arg2, IM_S32_MIN, IM_S32_MAX); }
            if (op == '-') { *(ImS32*)output = ImSubClampOverflow(*(const ImS32*)arg1, *(const ImS32*)arg2, IM_S32_MIN, IM_S32_MAX); }
            return;
        case ImGuiDataType_U32:
            if (op == '+') { *(ImU32*)output = ImGuiClampOverflow(*(const ImU32*)arg1, *(const ImU32*)arg2, IM_U32_MIN, IM_U32_MAX); }
            if (op == '-') { *(ImU32*)output = ImSubClampOverflow(*(const ImU32*)arg1, *(const ImU32*)arg2, IM_U32_MIN, IM_U32_MAX); }
            return;
        case ImGuiDataType_S64:
            if (op == '+') { *(ImS64*)output = ImGuiClampOverflow(*(const ImS64*)arg1, *(const ImS64*)arg2, IM_S64_MIN, IM_S64_MAX); }
            if (op == '-') { *(ImS64*)output = ImSubClampOverflow(*(const ImS64*)arg1, *(const ImS64*)arg2, IM_S64_MIN, IM_S64_MAX); }
            return;
        case ImGuiDataType_U64:
            if (op == '+') { *(ImU64*)output = ImGuiClampOverflow(*(const ImU64*)arg1, *(const ImU64*)arg2, IM_U64_MIN, IM_U64_MAX); }
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    float maXMax = (v_min >= v_max) ? FLT_MAX : v_max;
    ImGuiSliderFlags max_flags = flags | ((max_min == maXMax) ? ImGuiSliderFlags_ReadOnly : 0);
    value_changed |= DragScalar("##max", ImGuiDataType_Float, v_current_max, v_speed, &max_min, &maXMax, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    ImGuiSliderFlags max_flags = flags | ((max_min == maXMax) ? ImGuiSliderFlags_ReadOnly : 0);
    value_changed |= DragInt("##max", v_current_max, v_speed, max_min, maXMax, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, float width, const char* format)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.Frame = ImLerp(it_anim->second.Frame, colFrame, 1.0f / ImGui_ANIMATIONS_SPEED * ImGui::GetIO().DeltaTime);
    it_anim->second.Grab = ImLerp(it_anim->second.Grab, colGrab, 1.0f / ImGui_ANIMATIONS_SPEED * ImGui::GetIO().DeltaTime);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, float width, const char* format)
{
    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, width, format);
}

bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, float width, const char* format)
{
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const int buf_displaYMax_length = 2 * 1024 * 1024;
    ...
    if (render_cursor || render_selection)
    {
        ...
        // We test for 'buf_displaYMax_length' as a way to avoid some pathological cases (e.g. single-line 1 MB string) which would make ImDrawList crash.
        if (is_multiline || (buf_display_end - buf_display) < buf_displaYMax_length)
        {
